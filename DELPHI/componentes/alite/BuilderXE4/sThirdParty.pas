  ...
  begin
    ...
    if SkinData.CustomColor then
      ...
      if (SkinData.SkinManager <> nil) and SkinData.SkinManager.IsValidSkinIndex(DrawData.SkinIndex) then
        DrawData.BGColor := GetControlColor(Button)// SkinData.CommonSkinData.gd[DrawData.SkinIndex].Props[0].Color
      else
...
//  ThirdPartySkipForms.Add('TQRStandardPreview');
  ...
  ThirdPartySkipForms.Add('TfcxSliceFieldsPopup'); {FastCube popup} 
  ThirdPartySkipForms.Add('TfcxMeasurePopup'); {FastCube popup} 
  ThirdPartySkipForms.Add('TfcxFilterPopup'); {FastCube popup} 
  ThirdPartySkipForms.Add('TAdvSmoothMessageDialogForm');
{$IFDEF DEVEX2011}
  ...
  ThirdPartySkipForms.Add('TPopupDBTreeView');
{$ENDIF}
